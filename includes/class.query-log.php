...
<?php
		...
		public function pagination( $location = 'top', $id = '' ) {
			...
			if( 'daterange' != $log_type ) {
				...
				if( $this->have_entries() ) {
					...
					$custom_results = $wpdb->get_results($query);
					$custom_count = count( $custom_results );
					$output = '<span class="displaying-num">'.$custom_count.' entries</span>';
				}
			}
		...
		public function get_display() {
			...
			if ( $this->is_admin )
				$table_class = 'mpt-mycred-table mycred-table wp-list-table widefat fixed striped users';
				$get_custom_date = get_option( 'mpt_custom_date' );
				$start_date = isset( $get_custom_date['start'] ) ? date( 'Y-m-d', $get_custom_date['start'] ) : '';
				if( $start_date ) {
				...
				$end_date = isset( $get_custom_date['end'] ) ? date( 'Y-m-d', $get_custom_date['end'] ) : '';
				...
				?>
				<input type="date" name="mpt-start-data" class="mpt-start-date" value="<?php echo $start_date; ?>">
				<input type="date" name="mpt-end-data" class="mpt-end-date" value="<?php echo $end_date; ?>">
				<input type="button" class="mpt-search-btn button button-secondary" value="<?php echo __( 'Search', MPT_TEXT_DOMAIN ); ?>">
				<input type="button" class="mpt-export-button button button-primary" value="Export">
				<?php
...
<table class="' . apply_filters( 'mycred_log_table_classes', $table_class, $this ) . '" cellspacing="0" cellpadding="0">
	...
	<tbody id="the-list">';
			...
			$custom_date_range = isset( $_GET['show'] ) ? $_GET['show'] : '';

			if( $custom_date_range && 'daterange' == $custom_date_range ) {
				...
				$query = $wpdb->prepare("
					...
					LIMIT %d, %d
					", $start_date, $end_date, $start_offset, $per_page);
			
				$custom_results = $wpdb->get_results($query);
				$this->results = $custom_results; 
			} 

			if( 'daterange' != $custom_date_range || ! $custom_date_range ) {
				delete_option( 'mpt_custom_date' );
			}
			...
			foreach ( $this->headers as $column_id => $column_name ) {
				...
				switch ( $column_id ) {
					...
					case 'username' :
						...
						$user_name = Mycred_Point_Transfer::mpt_get_user_name( $log_entry->user_id );
					...
					case 'territory' :
						...
						$c_user_id = $log_entry->user_id;
						$c_time = $log_entry->time;
						...
						$territory_id = isset( $results[0]->territory_id ) ? $results[0]->territory_id : '';
						$territory_id = Mycred_Point_Transfer::mpt_get_user_name( $territory_id );

						if( $territory_id ) {
							$content = $territory_id;	
						} else {
							$content = '';
						}
					...
					case 'recipeint' :
						...
						$results = $wpdb->get_results($query);
						$recipient = isset( $results[0]->recipient_id ) ? intval( $results[0]->recipient_id ) : '';

						$recipient_name = Mycred_Point_Transfer::mpt_get_user_name( $recipient );
					...
					case 'creds' :
						...
						$content = str_replace( 'Points', '', $content );
